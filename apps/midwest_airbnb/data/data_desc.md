...
| `host_is_superhost` | boolean [t=true; f=false] | Whether the host qualifies as a "superhost" on Airbnb. |
...
| `review_scores_rating` | numeric | The average rating given on a review of the listing. |
| `reviews_per_month` | numeric | The number of reviews the listing has averaged per month. |
...
| `estimated_revenue_l365d` | numeric | The estimated revenue of the listing in the last 365 days. |
| `amenities_count` | integer | The number of items in each listing's `amenities` list. |

Continue the table for the remaining 23 columns (Assignment 05):
 , , , .
